...
На рисунке  ~\ref{componetdiag:image} представлена диаграмма компонентов для проектируемой системы. На данной диаграмме изображёны:
\begin{enumerate}
 	\item Компонент "<Меню"> отвечает за отображение блюд и напитков, их категории, цен, описания и состава. Данный компонент взаимодействует с Модулем управления заказа и Модулем управления составом блюд. В первом случаи связь нужна для предоставления сотрудникам ресторана возможности выбора блюд при формировании заказа. Во втором целью взаимодейтсвия является получение информации об ингредиентах и КБЖУ. Также данные компонента "<Меню"> хранятся в Базе данных.
...
При вводе пароля повара, откроется окно "<Меню">, представленное классом ManagerMenu. В рамках данного окна создаются вкладки "<Категории меню">, "<Добавить ингредиент">, "<Добавить блюдо">, "<Создать тех. катру">, "<Просмотр тех. карту">, представленные классами Category, Ingredients, Dishes, New и Old соответственно.  
 ...
 Если должность пользователя - "<официант">, то появится окно "<Cтолы">, класс которого - Tabels. При нажатии на кнопку с номером стола, инициализируется модуль управления заказом, представленный классом Main. При закрытии стола и оплате заказа происходит активация модуля управления платежами (класс Payment).
 ...
 \begin{landscape}
 	\begin{figure}[ht]
 		\centering
 		\includegraphics[width=1\linewidth]{diagrammaclassov}
